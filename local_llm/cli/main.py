...
def run_batching_test(model_name="mistral-7b-instruct"):
    generate, tokenizer = get_model_and_tokenizer(model_name)
    output_length = 1
    max_batch_size = 256+8
    max_input_length = max_batch_size
    step_size = 20
    results = [("batch_size", "input_length", "output_length", "runtime")]
    try:
        for input_length  in range(step_size, max_input_length+1, step_size):
            try:
                for batch_size in range(step_size, max_batch_size+1, step_size):
                    # model_pipeline = llm.create_pipeline(model, tokenizer, batch_size)
                    # generate = functools.partial(llm.generate, model_pipeline)
                    results.append(test_batching(generate, tokenizer, input_length, output_length, batch_size))
        ...
        columns = ["batch_size"]
        columns += [f"{idx}" for idx in range(step_size, max_input_length+1, step_size)]
        ...
        for batch_size, result in tmp.items():
            ...
            for input_length in range(step_size, max_input_length+1, step_size):
                ...
                tmp_result += [result.get(input_length, -1)]
